...
import { usePermission } from '../../../hooks/usePermission'; // ĐẢM BẢO ĐƯỜNG DẪN NÀY ĐÚNG

const TaskNoteSection = ({ isOpen, onClose, taskId }) => {
  // Lấy ID user hiện tại để xác định tác giả của ghi chú
  const currentUserId = parseInt(localStorage.getItem("user_id") || "1", 10);

  // --- GỌI HOOK PHÂN QUYỀN ---
  const { hasPermission } = usePermission();
  const canView = hasPermission("task_notes.view");
  const canCreate = hasPermission("task_notes.create");
  const canUpdate = hasPermission("task_notes.update");
  const canDelete = hasPermission("task_notes.delete");
  ...
  const fetchNotes = async () => {
    // Chặn fetch nếu không có taskId, modal đang đóng, HOẶC không có quyền xem
    if (!taskId || !isOpen || !canView) return; 
  ...
  useEffect(() => {
    fetchNotes();
  }, [taskId, isOpen, canView]);

  const handleSend = async (e) => {
    e.preventDefault();
    if (!canCreate) { toast.error("Bạn không có quyền thêm thảo luận!"); return; }
    if (!content.trim()) { setError("Nội dung không được để trống"); return; }
    if (content.length > 1000) { setError("Nội dung không được vượt quá 1000 ký tự"); return; }
    setError('');
  ...
  const handleUpdate = async (noteId) => {
    ...
    if (editContent.length > 1000) { toast.error("Vượt quá 1000 ký tự"); return; }

    try {
      await api.put(`/task-notes/${noteId}`, { content: editContent });
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white w-full max-w-2xl rounded-[5px] shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar text-[11px]">
          {!canView ? (
             <div className="text-center py-10">
               <span className="material-symbols-outlined text-red-300 text-5xl mb-2">lock</span>
               <p className="text-red-500 font-bold text-[13px]">Bạn không có quyền xem thảo luận công việc này.</p>
             </div>
          ) : isLoading ? (
            ...
            notes.map((note) => {
              ...
              const showEdit = canUpdate || isOwner;
                ...
                <div key={note.id} className="bg-white border border-slate-200 rounded-[5px] p-4 shadow-sm group hover:border-blue-200 transition-colors">
                  <div className="flex justify-between items-start mb-2 border-b border-slate-100 pb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-[11px] font-bold text-slate-700">{note.createdByName || `Người dùng #${note.userId}`}</p>
                        <p className="text-[10px] text-slate-400">{new Date(note.createdAt).toLocaleString('vi-VN', { dateStyle: 'short', timeStyle: 'short' })}</p>
                      </div>
                    </div>
                    
                    {/* KHỐI NÚT THAO TÁC (ẨN/HIỆN THEO QUYỀN) */}
                    {(showEdit || showDelete) && (
        ...
        {/* CHỈ HIỂN THỊ FORM NHẬP NẾU CÓ QUYỀN CREATE */}
        {canCreate && canView && (
