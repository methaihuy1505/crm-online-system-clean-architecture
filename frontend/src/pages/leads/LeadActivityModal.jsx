...
const LeadActivityModal = ({ isOpen, onClose, onSave, leadId, leadName }) => {
  ...
  const getInitialForm = () => ({
    ...
    endDate: "",
    location: "",
    isCompleted: true, // Mặc định ghi nhanh là đã hoàn thành
    isPriority: false,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        // Chuẩn camelCase
        parentId: Number(leadId),
        parentType: "LEAD",
        createdBy: currentUserId,
        updatedBy: currentUserId,

        // Chuẩn snake_case (Backup cho backend)
        parent_id: Number(leadId),
        parent_type: "LEAD",
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-[700px] flex flex-col animate-slide-up border border-slate-200">
        ...
        <div className="p-4 border-b border-slate-100 bg-slate-50 rounded-t-xl flex justify-between items-center">
          <div>
            ...
            <p className="text-[11px] text-slate-500 mt-0.5">Tiềm năng: <strong className="text-blue-600">{leadName}</strong></p>
          </div>
        ...
        <div className="p-4 flex flex-wrap gap-2 border-b border-slate-100 bg-white">
          ...
          <button type="button" onClick={() => handleChange({ target: { name: 'activityType', value: 'EMAIL_TRANSACTION' }})} className={`flex-1 min-w-[110px] py-2 rounded-lg flex justify-center items-center gap-1.5 text-[11px] font-bold transition-all border ${formData.activityType === 'EMAIL_TRANSACTION' ? 'bg-teal-50 border-teal-200 text-teal-700 shadow-sm' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
            <Mail size={14} /> Email G.Dịch
          </button>
        </div>

        <form id="quickActivityForm" onSubmit={handleSubmit} className="p-5 space-y-4 text-[12px] bg-slate-50/50">
          
          {/* CHỦ ĐỀ CHUNG */}
          <div className="space-y-1.5">
            <label className="font-bold text-slate-700">Chủ đề <span className="text-red-500">*</span></label>
            <input name="subject" value={formData.subject} onChange={handleChange} placeholder="Ví dụ: Gọi điện tư vấn lần 1, Gửi file báo giá..." className={`w-full px-3 py-2.5 rounded-lg border outline-none transition-all ${errorClass('subject')}`} />
            {renderErr('subject')}
          </div>

          {/* DYNAMIC FIELDS DỰA THEO TAB */}
          ...
          <div className="space-y-1.5 pt-2">
            ...
            <textarea name="description" value={formData.description} onChange={handleChange} rows="3" placeholder="Nhập nội dung chi tiết trao đổi hoặc file đính kèm..." className={`w-full px-3 py-2.5 rounded-lg border outline-none resize-none bg-white ${errorClass('description')}`}></textarea>
          ...
          <div className="flex items-center gap-6 pt-2">
            <label className="flex items-center gap-2 font-bold text-slate-700 cursor-pointer">
              <input type="checkbox" name="isCompleted" checked={formData.isCompleted} onChange={handleChange} className="w-4 h-4 text-blue-600 rounded" />
              Đã hoàn thành
            </label>
            <label className="flex items-center gap-2 font-bold text-amber-700 cursor-pointer">
              <input type="checkbox" name="isPriority" checked={formData.isPriority} onChange={handleChange} className="w-4 h-4 text-amber-500 rounded" />
              Đánh dấu quan trọng
            </label>
          </div>
        ...
        <div className="p-4 border-t border-slate-100 bg-white flex justify-end gap-3 rounded-b-xl">
          <button type="button" onClick={onClose} disabled={isSubmitting} className="px-5 py-2.5 text-xs font-bold text-slate-600 hover:bg-slate-100 border border-slate-200 rounded-lg transition-colors outline-none">
            Hủy bỏ
          </button>
          <button type="submit" form="quickActivityForm" disabled={isSubmitting} className="px-6 py-2.5 bg-primary text-white rounded-lg text-xs font-bold shadow-md hover:bg-blue-800 transition-all outline-none">
