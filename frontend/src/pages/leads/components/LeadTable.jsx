...
const LeadTable = ({
  ...
  const getStatusStyle = (statusName) => {
    ...
    if (statusName === "Đã chuyển đổi") return "bg-green-100 text-green-700 border-green-200 shadow-sm";
          ...
          <tbody className="divide-y divide-slate-50">
            {isLoading ? (
              ...
              filteredLeads.map((lead, index) => {
                ...
                const rowBaseStyle = isConverted ? "bg-slate-50/50 hover:bg-slate-100 opacity-80 grayscale-[20%]" : "bg-white hover:bg-slate-50";
                
                return (
                  <tr
                    key={lead.id}
                    data-index={index}
                    onClick={() => onRowClick(lead, index)}
                    onDoubleClick={() => canView && onOpenDetail(lead)} 
                    className={`transition-colors group cursor-pointer ${rowBaseStyle} ${
                      selectedLeadForRow?.id === lead.id ? "bg-primary/5 ring-1 ring-primary/20" : ""
                    }`}
                  >
                    <td className="px-6 py-2">
                      <div className="flex items-center gap-3">
                        ...
                        >
                          {lead.companyName ? <Building2 size={18} /> : <User size={18} />}
                        </div>
                        <div>
                          <p className={`font-bold text-sm line-clamp-1 ${isConverted ? "text-slate-600" : "text-slate-900"}`}>
                            {lead.fullName}
                          </p>
                    ...
                    <td className="px-6 py-2">
                      <div className="space-y-1">
                        <div className={`text-xs font-medium truncate max-w-[150px] ${isConverted ? "text-slate-500" : "text-slate-800"}`}>
                          {lead.phone || "---"}
                        </div>
                    ...
                    <td className="px-6 py-2">
                      <p
                        className={`text-xs font-bold ${lead.expectedRevenue ? (isConverted ? "text-primary/70" : "text-primary") : "text-slate-400"}`}
                      >
                        {formatCurrency(lead.expectedRevenue)}
                      </p>
                    </td>
                    <td className="px-6 py-2">
                      <p className={`text-[10px] font-medium ${isConverted ? "text-slate-500" : "text-slate-800"}`}>
                        MST: {lead.taxCode || "---"}
                      </p>
                    ...
                    <td className="px-6 py-2 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {isConverted ? (
                          <div 
                            className="p-1.5 text-slate-300 cursor-not-allowed bg-slate-50 rounded-lg flex items-center justify-center" 
                            title="Hồ sơ đã khóa (Chuyển đổi thành công)"
                          >
                          ...
                          <>
                            {canUpdate && (
                              <button
                                onClick={(e) => { e.stopPropagation(); onOpenEdit(lead); }}
                            ...
                            {canDelete && (
                              <button
                                onClick={(e) => { e.stopPropagation(); onDelete(lead.id, lead.fullName); }}
