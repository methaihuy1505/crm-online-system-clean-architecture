...
export const CallTable = ({ data, onEdit, onDelete, onToggleCompleted, onTogglePriority, onSortCallType, onSortParentType, canView, canUpdate, canDelete, selectedRow, onRowClick, onRowDoubleClick }) => (
  <table className="w-full text-left border-collapse">
    <thead className="bg-slate-50 border-b border-slate-200">
      <tr>
        <th className="p-3 font-semibold text-slate-600 sticky top-0 bg-slate-50">Chủ đề</th>
        <th className="p-3 font-semibold text-slate-600 sticky top-0 bg-slate-50">Ngày gọi</th>
        <th className="p-3 font-semibold text-slate-600 text-center cursor-pointer hover:bg-slate-100 sticky top-0 bg-slate-50" onClick={onSortCallType}>
          <div className="flex items-center justify-center gap-1">Loại <ChevronsUpDown className="w-3 h-3 text-slate-400" /></div>
        </th>
      ...
      {data.map((act, index) => (
        ...
        >
          ...
          <td className="p-3 text-center whitespace-nowrap">
            {act.callType === "INBOUND" ? <span className="text-[11px] px-2 py-0.5 bg-blue-50 text-blue-700 rounded">Đến</span> : <span className="text-[11px] px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded">Đi</span>}
          </td>
