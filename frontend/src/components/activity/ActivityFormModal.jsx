...
const ActivityFormModal = ({ isOpen, onClose, onSave, initialData }) => {
    ...
    switch (formData.activityType) {
      case "CALL":
        if (!formData.activityDate)
          newErrors.activityDate = "Ngày gọi không được để trống.";
        if (!formData.callType)
          newErrors.callType = "Loại cuộc gọi không được để trống.";
        if (!formData.callResult || !formData.callResult.trim())
          newErrors.callResult = "Kết quả cuộc gọi không được để trống.";
        if (parseInt(formData.duration, 10) <= 0)
          newErrors.duration = "Thời lượng cuộc gọi phải lớn hơn 0.";
        break;
      ...
      case "NOTE":
        if (!formData.activityDate)
          newErrors.activityDate = "Ngày ghi chú không được để trống.";
        if (!formData.description || !formData.description.trim())
          newErrors.description =
            "Nội dung ghi chú (Mô tả) không được để trống.";
        break;

      case "EMAIL_QUOTE":
      case "EMAIL_TRANSACTION":
        if (!formData.activityDate)
          newErrors.activityDate = "Ngày gửi Email không được để trống.";
        break;
  ...
  const renderError = (field) =>
    errors[field] && (
      <p className="text-red-500 text-[11px] mt-1 font-medium animate-pulse">
        {errors[field]}
      </p>
    );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      ...
      <div className="bg-white w-full max-w-4xl rounded-[5px] shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center rounded-t-[5px]">
          <div>
            ...
            <p className="text-[11px] text-slate-500 mt-1 italic">
              <span className="text-red-500 font-bold">*</span>: Thông tin bắt
              buộc, không được để rỗng.
            </p>
          </div>
          <button
            onClick={onClose}
            className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-200 rounded-[5px] transition-colors"
          >
        ...
        <form
          ...
          className="p-6 space-y-5 overflow-y-auto flex-1 custom-scrollbar text-[11px]"
        >
          {/* LOẠI HOẠT ĐỘNG */}
          <div className="space-y-2">
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">
              Loại hoạt động
            </label>
          ...
          <div className="bg-white space-y-4">
            ...
            {/* --- NOTE & EMAIL --- */}
            {["NOTE", "EMAIL_QUOTE", "EMAIL_TRANSACTION"].includes(
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 pt-2 border-t border-slate-100 mt-4">
              <div className="md:col-span-4 space-y-1">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full px-3 py-2 text-[11px] bg-white border rounded-[5px] outline-none focus:ring-1 transition-all ${errorClass("activityDate")}`}
                />
              ...
              <div className="md:col-span-4 space-y-1">
                ...
                <input
                  ...
                  disabled={
                    formData.activityType !== "CALL" &&
                    formData.activityType !== "EMAIL_QUOTE"
                  }
                  className={`w-full px-3 py-2 text-[11px] border rounded-[5px] outline-none focus:ring-1 transition-all ${formData.activityType === "CALL" || formData.activityType === "EMAIL_QUOTE" ? "bg-red-50 text-red-900 border-red-200" : "bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed"}`}
                />
              </div>
              <div className="md:col-span-4 space-y-1">
                <label className="block text-[11px] font-bold text-slate-600 whitespace-nowrap">
                ...
                <div className="flex flex-col gap-2 mt-2">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={handleChange}
                      className="w-4 h-4 text-blue-600 rounded-[5px] border-slate-300 focus:ring-blue-500 cursor-pointer"
                    />
                    <label
                      htmlFor="isCompleted"
                      className="text-[11px] font-semibold text-slate-700 cursor-pointer whitespace-nowrap"
                    >
                      Đã hoàn thành
                    </label>
                  </div>
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={handleChange}
                      className="w-4 h-4 text-amber-500 rounded-[5px] border-slate-300 focus:ring-amber-500 cursor-pointer"
                    />
                    <label
                      htmlFor="isPriority"
                      className="text-[11px] font-semibold text-slate-700 cursor-pointer whitespace-nowrap"
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3 p-4 bg-slate-50 rounded-[5px] border border-slate-200">
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) => {
                  handleChange(e);
                  // Reset ID đối tượng về rỗng khi đổi Loại liên kết
                  handleChange({ target: { name: "parentId", value: "" } });
                }}
                className="w-full px-3 py-2 text-[11px] bg-white border border-slate-300 rounded-[5px] outline-none focus:ring-1"
              >
            ...
            {/* 2. ID ĐỐI TƯỢNG (Dropdown hiển thị Tên theo Loại) */}
            <div className="space-y-1">
              <label className="block text-[11px] font-bold text-slate-500 uppercase whitespace-nowrap">
              ...
              <select
                ...
                onChange={handleChange}
                className={`w-full px-3 py-2 text-[11px] bg-white border rounded-[5px] outline-none focus:ring-1 ${errorClass("parentId")}`}
              >
                <option value="">-- Chọn đối tượng --</option>
                {formData.parentType === "LEAD" &&
                  MOCK_LEADS_DATA.map((item) => (
                    <option key={item.id} value={item.id}>
                      {item.fullName || item.name} (ID: #{item.id})
                    </option>
                  ))}
                {formData.parentType === "CUSTOMER" &&
                  MOCK_CUSTOMERS_DATA.map((item) => (
                    <option key={item.id} value={item.id}>
                      {item.name || item.fullName} (ID: #{item.id})
                    </option>
                  ))}
                {formData.parentType === "OPPORTUNITY" &&
                  MOCK_OPPORTUNITIES_DATA.map((item) => (
                    <option key={item.id} value={item.id}>
                      {item.name || item.fullName} (ID: #{item.id})
                    </option>
                  ))}
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={handleChange}
                placeholder="Task ID..."
                className="w-full px-3 py-2 text-[11px] bg-white border border-blue-200 rounded-[5px] outline-none focus:ring-1 focus:ring-blue-500/30"
              />
            </div>

            {/* 4. ID NGƯỜI TẠO (Dropdown hiển thị Tên Nhân viên) */}
            <div className="space-y-1">
              <label className="block text-[11px] font-bold text-slate-500 uppercase whitespace-nowrap">
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={handleChange}
                className="w-full px-4 py-2 text-[11px] bg-white border border-slate-300 rounded-[5px] outline-none focus:ring-1 text-slate-700"
              >
                <option value="">-- Chọn liên hệ (Tùy chọn) --</option>
                {(formData.parentType === "CUSTOMER" && formData.parentId
                  ? MOCK_CONTACTS_DATA.filter(
                      (c) => Number(c.customerId) === Number(formData.parentId),
                    )
                  : MOCK_CONTACTS_DATA
                ).map((item) => (
                  <option key={item.id} value={item.id}>
        ...
        <div className="p-4 border-t border-slate-100 bg-white flex justify-end gap-3 rounded-b-[5px]">
          <button
            ...
            className="px-5 py-2 text-[11px] font-bold text-slate-600 hover:bg-slate-100 rounded-[5px] transition-colors"
          >
            Hủy (Ctrl + H)
          </button>
          <button
            ...
            form="activityForm"
            disabled={isSubmitting}
            className="px-6 py-2 bg-blue-800 text-white rounded-[5px] text-[11px] font-bold shadow hover:bg-blue-900 flex items-center gap-2"
          >
            {isSubmitting ? (
              <>
